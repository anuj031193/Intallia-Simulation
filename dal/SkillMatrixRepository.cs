...
namespace JobSimulation.DAL
{
    public class SkillMatrixRepository
    {
        ...
        public async Task UpsertSkillMatrixAsync(SkillMatrix skillMatrix, string userId)
        {
            using var connection = new SqlConnection(_connectionString);

            // Check if the record already exists
            var existingRecord = await connection.QueryFirstOrDefaultAsync<SkillMatrix>(
                "SELECT * FROM SkillMatrix WHERE ActivityId = @ActivityId AND TaskId = @TaskId",
                new { skillMatrix.ActivityId, skillMatrix.TaskId });

            if (existingRecord != null)
            {
                // Perform an update if the record exists
                var updateQuery = @"
            UPDATE SkillMatrix 
            ...
            {
                // Perform an insert if the record does not exist
                var insertQuery = @"
            ...
            VALUES (@ActivityId, @TaskId, @HintsChecked, @TotalTime, @AttemptstoSolve, @Status, 
                ...
                await connection.ExecuteAsync(insertQuery, new
                {
                    ...
                    skillMatrix.AttemptstoSolve,
                    skillMatrix.Status,
                    CreateBy = skillMatrix.CreateBy ?? userId,
                    CreateDate = skillMatrix.CreateDate != default(DateTime) ? skillMatrix.CreateDate : DateTime.Now,
                    ModifyBy = userId,
                    ModifyDate = DateTime.Now,
                    skillMatrix.TaskAttempt
                });
        ...
        public async Task<int> GetTotalTimeForTaskAsync(string activityId, string taskId)
        {
            using var connection = new SqlConnection(_connectionString);
        ...
        //        skillMatrix.ModifyDate = DateTime.Now;
